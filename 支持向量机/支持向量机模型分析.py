...
def build_svm(X, y):
    """SVM模型训练与超参数优化
    RBF核 + class_weight='balanced' + 网格搜索C和gamma
    5折交叉验证，ROC-AUC为指标
    ...
    print("\n[GridSearchCV] 搜索最优参数 (RBF核, class_weight='balanced')...")
    grid = GridSearchCV(
    ...
    # RBF核无法直接提取系数，报告支持向量数
    print(f"\n支持向量数: {sum(model.n_support_)} ({model.n_support_[0]}正常, {model.n_support_[1]}缺陷)")

    return model, X_train_s, X_test_s, y_train, y_test, scaler
...
def plot_roc(y_test, y_proba, auc_val):
    ...
    plt.plot(fpr, tpr, color='darkred', lw=2, label=f'ROC (AUC={auc_val:.3f})')
    plt.fill_between(fpr, tpr, alpha=0.15, color='darkred')  # AUC积分底纹
    ...
    plt.title('图4-6 SVM ROC曲线', fontsize=14, fontweight='bold')
    plt.legend(loc='lower right')
    plt.grid(True, alpha=0.3)
    optimal_idx = np.argmax(tpr - fpr)
    plt.plot(fpr[optimal_idx], tpr[optimal_idx], 'ro', markersize=8)
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, '46_SVM_ROC曲线.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("[OK] ROC曲线已保存")
...
def plot_confusion_matrix(y_test, y_pred):
    ...
    plt.title('图4-7 SVM混淆矩阵热力图', fontsize=14, fontweight='bold')
    plt.tight_layout()
    plt.savefig(os.path.join(FIGURES_DIR, '47_SVM混淆矩阵热力图.png'), dpi=300, bbox_inches='tight')
    plt.close()
    print("[OK] 混淆矩阵已保存")
...
def main():
    ...
    print("=" * 60)
    print("【支持向量机模型构建与性能评估（RBF核+类别平衡）】")

    X, y, _ = load_and_preprocess_data()
    model, X_train_s, X_test_s, y_train, y_test, scaler = build_svm(X, y)
    ...
    print(f"AUC={metrics['auc']:.4f}, F1={metrics['f1']:.4f}, "
          f"核函数=RBF, C={model.C}, gamma={model.gamma}, "
